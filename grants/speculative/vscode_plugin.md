...
VS Code plugin/extension is represented in the form of a sidebar with control panel. It allows to manage accounts, setup a development chain, install Substrate.
The extension parses node config and recognizes transaction types and inputs in a given chain. The control panel allows developers to send transactions to the chain and see logs and returned results from the executed transactions.
A short and clear guide on setup and usage of the plugin is provided.
 
 
*  Milestone 2 – Smart-Contracts – 15 working days – 7000 USD
The plugin contains controls for deploying and calling smart-contracts in a current development chain. The plugin enables verification of transactions by checking for critical bugs.
The guide is extended.
