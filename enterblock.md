...
* Introduction
  ...
  * Relay Nodes : enterBlock can support any other parachains infrastructure with HTTP endpoints. If clients are interested in serving API requests to and from one of our preconfigured other parachains such as Kusama, ChainX and other Parachains or a developer who wants to connect their application to a blockchain(s), these are the blockchain networks the Pocket Core protocol currently supports in Testnet phase one.
